...
def main():
    ...
    # Constant step size (1a)
    points_const, values_const = gradient_descent(f, grad_f, eta=lambda t: eta_const(
        t, c=0.2), initial_point=initial_point, max_iter=max_iter)
    results['const'] = (values_const[-1], min(values_const))

    # Decreasing step size (1b)
    points_sqrt, values_sqrt = gradient_descent(f, grad_f, eta=lambda t: eta_sqrt(
        t, c=0.5), initial_point=initial_point, max_iter=max_iter)
    results['sqrt'] = (values_sqrt[-1], min(values_sqrt))

    # Multi-step step size (1c)
    points_multistep, values_multistep = gradient_descent(
        f, grad_f, eta=lambda t: eta_multistep(
            t, milestones=[30, 50, 80], c=0.5, eta_init=1.0),
